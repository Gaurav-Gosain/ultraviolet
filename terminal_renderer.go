...
func (s *TerminalRenderer) transformLine(newbuf *RenderBuffer, y int) {
	...
	//      EL. This is the original check.
	//   2. Any cell on the new or old line is non-clearable, for example
	//      a kitty text-sizing multicell anchor or its CUF spill cell.
	...
	// (nbsp) so the trailing-cell guard tripped. That forced the slow
	// path on every row and made multicell-heavy frames significantly
	// slower than they need to be. The scan is O(width) per line and
	// runs once per frame; in practice it's dominated by the SGR
	// emission it lets us short-circuit on rows that don't need
	// protection.
